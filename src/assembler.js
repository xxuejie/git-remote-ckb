...
// TODO: allow customized signing command
const SIGNING_COMMAND = path.normalize(path.join(__dirname, "..", "sign"));
...
async function signAndSendTransactionSkeleton(skeleton, address) {
  const signatures = skeleton
    ...
    .map(({ message }) => {
      ...
      const signature = execSync(`${SIGNING_COMMAND} ${message} ${address}`)
        .toString("utf8")
        .trim();
      return signature;
    })
...
async function createRepository(address) {
  let skeleton = TransactionSkeleton({
    cellProvider: new CkbIndexerQuerier(CKB_INDEXER_URL),
  });
  ...
  skeleton = await common.injectCapacity(
    skeleton,
    [address],
    BigInt(146) * BigInt(100000000),
    address
  );
...
async function locateRepositoryCell(remoteUrl) {
  ...
  if (!result2.done) {
    throw new Error(
      "More than one repository cell exists, something is severely wrong!"
    );
  }
...
async function sendToCkb(data, newTipHash, remoteUrl) {
  ...
  skeleton = await common.payFee(skeleton, [address], FEE);
  const oldWitnessArgs = new core.WitnessArgs(
    new Reader(skeleton.get("witnesses").get(0))
  );
  const witnessArgs = {
    lock: new Reader(oldWitnessArgs.getLock().value().raw()),
    input_type: new Reader(data),
  };
  const witnessBuffer = core.SerializeWitnessArgs(
...
async function downloadFromCkb(remoteUrl, currentTipHash = null) {
  ...
  while (true) {
    ...
    if (
      txTipHash === INITIAL_TIP_HASH ||
      (currentTipHash && currentTipHash === txTipHash)
    ) {
      break;
    }
    const witnessArgs = new core.WitnessArgs(
      new Reader(tx.transaction.witnesses[0])
    );
    const bundle = new Reader(
      witnessArgs.getInputType().value().raw()
    ).serializeJson();
    bundles.unshift(bundle);
    txHash = tx.transaction.inputs[0].previous_output.tx_hash;
  }
  return bundles;
}
