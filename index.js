    ...
    switch (commands[0]) {
      ...
      case "fetch":
        ...
        try {
          localTipHash =
            "0x" +
            execSync("git rev-parse HEAD 2>/dev/null").toString("utf8").trim();
        } catch (e) {}
      ...
      case "push":
        ...
        const pushTipHash =
          "0x" + execSync("git rev-parse HEAD").toString("utf8").trim();
        await sendToCkb(data, pushTipHash, URL);
        execSync(
          `git update-ref refs/remotes/origin/master ${pushTipHash.substr(2)}`
        );
        break;
      case "":
        ...
        break;
      default:
        console.error("UNPROCESSED:", line);
        process.exit(1);
    }
  }
})().catch((e) => {
